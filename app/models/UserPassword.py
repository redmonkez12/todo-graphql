...
class UserPassword(SQLModel, table=True):
    ...
    created_at: Optional[datetime.datetime] = Field(sa_column=sa.Column(sa.DateTime(timezone=True), default=sa.func.now()))
    updated_at: Optional[datetime.datetime] = Field(sa_column=sa.Column(sa.DateTime(timezone=True), default=sa.func.now()))
